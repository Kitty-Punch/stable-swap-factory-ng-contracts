...
interface IStableKittyFactoryNG {
    ...
    function base_pool_list(uint256 _index) external view returns (address);

    function base_pool_count() external view returns (uint256);

    /*
    function base_pool_data(address _base_pool)
        external
        view
        returns (BasePoolArray);
    */

    // asset -> is used in a metapool?
    function base_pool_assets(address _asset) external view returns (bool);
    ...
    function markets(uint256 _id) external view returns (address[] memory);

    function market_counts(uint256 _id) external view returns (uint256);
    ...
    // Returns Number of wrapped coins, number of underlying coins
    ...
    // Returns Pool fee and admin fee as uint256 with 1e10 precision
    ...
    function set_metapool_implementation(
        uint256 _implementation_index,
        address _implementation
    ) external;
